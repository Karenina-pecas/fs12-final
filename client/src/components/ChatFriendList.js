...
export default function ChatFriendList() {
	...
	return (
		<div className='list-group users-list'>
			<div>
				<div>
					<h3 className='text-center my-4'>Please, select a friend to chat with!</h3>
				</div>
			</div>
			<div>
				{item
					...
					.map((list, index) => (
						<NavLink
							className='text-decoration-none d-flex justify-content-evenly my-3 ml-3 '
							to={`/chatPage/chat/${list.id}`}
							key={index}>
							<img className='imgfriend' src={process.env.PUBLIC_URL + `/img/${list.profile_photo}`} alt='' />
							<h4 className='my-3 blue'>{list.name}</h4>
						</NavLink>
					))}
			</div>
		</div>
